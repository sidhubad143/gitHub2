...
def github_list_repos(token: str) -> tuple[bool, list, str]:
    """Fetch all repos from GitHub account. Returns (success, repos_list, error_msg)"""
    try:
        all_repos = []
        page = 1
        while True:
...
def github_set_visibility(token: str, repo_url: str, private: bool) -> tuple[bool, str]:
    """Toggle repo visibility public/private via GitHub API."""
    try:
        m = re.search(r"github\.com[:/](.+?)(?:\.git)?$", repo_url)
        if not m:
            return False, "❌ Invalid GitHub URL."
        ...
        if r.status_code == 200:
            ...
            return True, f"✅ Repo is now **{status}**"
        err = r.json().get("message", r.text[:200])
        return False, f"❌ GitHub error: `{err}`"
    except Exception as e:
        return False, f"❌ Error: {e}"
...
def delete_path_in_repo(repo_folder: str, rel_path: str) -> tuple[bool, str]:
    """Delete a file or folder inside a workspace repo folder."""
    ...
    try:
        if os.path.isdir(target):
            ...
            return True, f"✅ Deleted folder `{repo_folder}/{rel_path}`"
        else:
            os.remove(target)
            return True, f"✅ Deleted file `{repo_folder}/{rel_path}`"
    except Exception as e:
        return False, f"❌ Delete failed: {e}"


def write_file_in_repo(repo_folder: str, rel_path: str, content: str) -> tuple[bool, str]:
    """Create or overwrite a file inside a workspace repo folder."""
    repo_folder = repo_folder.strip()
    rel_path    = rel_path.strip().lstrip("/\\")
    repo_dir    = os.path.join(WORK_DIR, repo_folder)
    if not os.path.exists(repo_dir):
    ...
    try:
        ...
        return True, f"✅ {verb} `{repo_folder}/{rel_path}`"
    except Exception as e:
