...
def git_push(directory, token, repo_url, branch="main"):
    try:
        ...
        # Step 6: push (retry with --no-thin if pack/object error)
        r = run(["git", "push", "-u", "origin", branch, "--force"])
        if r.returncode != 0:
            err = r.stderr.strip()
            # Retry with --no-thin flag for "index-pack failed" / object errors
            if any(x in err.lower() for x in ["index-pack", "unpack", "object", "pack"]):
                r = run(["git", "push", "-u", "origin", branch, "--force", "--no-thin"])
        os.chdir(orig)
        if r.returncode == 0:
...
def unzip_and_push(zip_path, token, repo_url, branch="main"):
    ...
    try:
        ...
        # Find actual content dir (skip single wrapper folder if present)
        contents = [i for i in os.listdir(temp_dir) if not i.startswith("__")]
        ...
        git_dir = os.path.join(push_dir, ".git")
        if os.path.exists(git_dir):
            shutil.rmtree(git_dir)

        # Verify there are actual files to push
        file_count = sum(len(files) for _, _, files in os.walk(push_dir))
        if file_count == 0:
            return False, "❌ ZIP appears to be empty — no files found."
