...
async def cb(client: Client, q: CallbackQuery):
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  RENAME PATH — Direct path rename inside repo (no folder picker needed)
    #  e.g. bad/badmunda.py → bad/newname.py
    # ══════════════════════════════════════════════════════════════════════════
        ...
        await db.set_state(uid, {"action": "rename_path_step1"})
        await q.message.reply(
            f"🔀 **Rename Path Inside Repo**\n\n"
            f"{workspace}\n\n"
            "**Step 1:** Send the **old path** in format:\n"
            "`repo_folder/path/to/file_or_dir`\n\n"
            "Example: `bad/badmunda.py` or `myrepo/utils/helper.py`"
        )
...
async def msg_handler(client: Client, msg: Message):
    ...
    # ── RENAME PATH: Step 1 — receive old path ────────────────────────────────
    elif action == "rename_path_step1":
        # Expect: "repo_folder/path/to/file"
        parts = text.strip().lstrip("/\\").split("/", 1)
        if len(parts) < 2:
            await msg.reply(
                "❌ Please include the repo folder + path.\n"
                "Example: `bad/badmunda.py` or `myrepo/utils/old.py`"
            )
            return
        repo_folder = parts[0]
        old_rel     = parts[1]
        await db.set_state(uid, {
            "action": "rename_path_step2",
            "repo_folder": repo_folder,
            "old_rel": old_rel,
        })
        await msg.reply(
            f"🔀 Renaming: `{repo_folder}/{old_rel}`\n\n"
            "**Step 2:** Send the **new path** (only the part inside the repo):\n"
            "Example: `badmunda_new.py` or `utils/new_helper.py`\n\n"
            "_Just the new relative path inside the same repo folder_"
        )

    # ── RENAME PATH: Step 2 — receive new path ────────────────────────────────
    elif action == "rename_path_step2":
