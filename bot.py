...
async def cb(client: Client, q: CallbackQuery):
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        await q.message.reply(
            ...
            "_(spaces will become dashes)_"
        )

    # ══════════════════════════════════════════════════════════════════════════
    #  DELETE GITHUB REPO — from main menu (pick from saved list)
    # ══════════════════════════════════════════════════════════════════════════
        ...
        if not repos:
            await q.message.reply(
                "❌ No repos in your list.\nAdd repos first via ➕ Add Repo.",
                reply_markup=main_keyboard()
            )
            return
        rows = []
        for i, r in enumerate(repos):
            ...
            rows.append([InlineKeyboardButton(
                f"🗑 {lock} {label}", callback_data=f"gh_del_repo:{i}"
            )])
...
async def msg_handler(client: Client, msg: Message):
    ...
    # ── CREATE REPO: name ────────────────────────────────────────────────────
        ...
        await status.edit_text(result)
        if ok and url:
            # Auto-add to saved repos
            added = await db.add_repo(uid, url, repo_name, private)
            if added:
                await msg.reply(
                    f"📌 Auto-saved to your repo list!\n"
                    f"Tap **📂 My Repos** to see it.",
                    reply_markup=main_keyboard()
                )
            else:
                await msg.reply("", reply_markup=main_keyboard())
    ...
    # ── CONFIRM GITHUB DELETE ─────────────────────────────────────────────────
    elif action == "confirm_gh_delete":
        if text.strip().upper() != "DELETE":
            await msg.reply(
                "❌ Type exactly `DELETE` (uppercase) to confirm.\n"
                "Or tap Cancel.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("🔙 Cancel", callback_data="go_home")
                ]])
        ...
        await status.edit_text(result)
        if ok:
            # Also remove from bot's saved list
            repos = await db.get_repos(uid)
            new_repos = [r for r in repos if r["url"] != repo_url]
            await db.set_repos(uid, new_repos)
    ...
    # ── CONFIRM CLEAN DATA ────────────────────────────────────────────────────
        ...
        if text.strip().upper() != "CLEAN":
            await msg.reply(
                "❌ Type exactly `CLEAN` (uppercase) to confirm.\n"
                "Or tap Cancel.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("🔙 Cancel", callback_data="go_home")
                ]])
