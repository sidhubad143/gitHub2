...
async def cmd_start(client: Client, msg: Message):
    ...
    text = (
        ...
        "**🐙 Push→GitHub** pushes all files directly to repo.\n\n"
        "👇 Choose:"
    )
...
async def cmd_help(client: Client, msg: Message):
    await msg.reply(
        ...
        "**🐙 Push→GitHub:**\n"
        "Pushes ALL files from folder directly into GitHub repo.\n"
        "Files appear in repo as-is, unzipped.\n\n"
        "**📦 Make ZIP:**\n"
...
async def cb(client: Client, q: CallbackQuery):
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  PUSH→GITHUB (files directly, not zip as release)
    #  Pushes all files from folder into GitHub repo via git
    # ══════════════════════════════════════════════════════════════════════════
        ...
        if not folders:
            ...
            return
        await q.message.reply(
            f"🐙 **Push Files → GitHub Repo**\n"
            f"🎯 Repo: `{active}`\n\n"
            "All files from the folder will be pushed directly.\n\n"
            "Tap the folder:",
            reply_markup=folder_picker_kb("pu"),  # pu:0, pu:1 …
        )
        ...
        status   = await q.message.reply(
            f"⏳ Pushing `{folder}` files to GitHub...\n"
            f"🎯 `{repo_url}`"
        )
        result = git.git_push(src, token, repo_url)
        await status.edit_text(result)
        await q.message.reply(
            f"✅ **Done!**\nAll files from `{folder}` pushed to:\n`{repo_url}`",
            reply_markup=main_keyboard(),
        )
        if uid != config.OWNER_ID:
            await alert_owner(client,
