...
from keyboards import (
    ...
    clone_type_keyboard, folder_picker_kb, gh_repo_action_kb
)
...
async def cb(client: Client, q: CallbackQuery):
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        token = await db.get_token(uid)
        if not token:
            await q.message.reply("❌ No GitHub token. Use 🔐 Set Token first.")
            return
        ...
        for r in chunk:
            ...
            rows.append([InlineKeyboardButton(
                f"{lock} {r['full_name'][:28]}", callback_data=f"gh_ri:{r['full_name'][:40]}:{page}"
            )])
        nav = []
        if page > 0:
        ...
        await status.edit_text(
            f"🌐 **GitHub Repos** ({total} total) — Page {page+1}:\n_Tap repo to manage_",
        ...
        desc       = rd.get("description") or "_(no description)_"
        url        = rd.get("html_url", "")
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        await q.message.reply(
            ...
            "**Step 1:** Send the **repo name:**\n_(spaces become dashes)_"
        )

    # ══════════════════════════════════════════════════════════════════════════
    #  DELETE GITHUB REPO — pick from saved list
    # ══════════════════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        await q.message.reply(
            ...
            "• ❌ Your GitHub token\n• ❌ All saved repos\n"
            "• ❌ Active repo setting\n• ❌ All pending states\n\n"
            "_(Workspace files on server are NOT deleted)_\n\nType `CLEAN` to confirm:",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Cancel", callback_data="go_home")]])
        )
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        await q.message.reply(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("📝 Add/Edit File",      callback_data="fm_add_file")],
        ...
        await q.message.reply("🗑 **Delete File/Folder**\n\nTap workspace folder:",
                              reply_markup=folder_picker_kb("fmd"))
        ...
        await q.message.reply("📝 **Add/Edit File**\n\nTap workspace folder:",
                              reply_markup=folder_picker_kb("fma"))
        ...
        if not folder:
            await q.message.reply("❌ Folder not found.")
            return
        await db.set_state(uid, {
            "action": "fm_multi_collecting", "repo_folder": folder,
            "files": {}, "current_path": None,
        })
        await q.message.reply(
            ...
            "**Start:** Send first file path now:"
        )
...
async def msg_handler(client: Client, msg: Message):
    ...
    if msg.document and action == "await_zip_file":
        ...
        # Download the ZIP file to workspace
        zip_path = os.path.join(config.WORK_DIR, f"_upload_{uid}.zip")
        try:
    ...
    # ── CONFIRM GH DELETE by name (from GitHub repos list) ──────────────────
    elif action == "confirm_gh_delete_name":
        if text.strip().upper() != "DELETE":
            await msg.reply(
                "❌ Type exactly `DELETE` to confirm. Or tap Cancel.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Cancel", callback_data="go_home")]])
            )
            return
        repo_url  = state["url"]
        ...
        if ok:
            ...
            await msg.reply("🗑 Removed from your saved list too.", reply_markup=main_keyboard())
        else:
            await msg.reply("", reply_markup=main_keyboard())

    # ── CONFIRM GH DELETE from saved list ────────────────────────────────────
        ...
        token    = await db.get_token(uid)
        await db.clear_state(uid)
        status = await msg.reply(f"⏳ Deleting `{name}` from GitHub...")
        ok, result = git.github_delete_repo(token, repo_url)
        await status.edit_text(result)
        if ok:
            repos = await db.get_repos(uid)
    ...
    # ── CREATE REPO: name ─────────────────────────────────────────────────────
        ...
        await msg.reply(
            ...
            "**Step 2:** Private or Public?\nReply `yes` = Private 🔒 or `no` = Public 🔓"
        )

    elif action == "create_repo_priv":
        repo_name = state["name"]
        private   = text.strip().lower() in ("yes", "y", "1", "true", "private")
        lock      = "🔒 Private" if private else "🔓 Public"
        await db.set_state(uid, {"action": "create_repo_desc", "name": repo_name, "private": private})
        await msg.reply(
            f"🆕 **{repo_name}** — {lock}\n\n"
            "**Step 3:** Send a short **description** (or `-` to skip):"
        )
        ...
        token     = await db.get_token(uid)
        await db.clear_state(uid)
        status = await msg.reply(f"⏳ Creating `{repo_name}` on GitHub...")
        ok, result, url = git.github_create_repo(token, repo_name, private=private, description=desc)
        await status.edit_text(result)
        if ok and url:
            added = await db.add_repo(uid, url, repo_name, private)
            note  = "📌 Auto-saved to your repo list!" if added else "ℹ️ Already in your list."
            await msg.reply(note, reply_markup=main_keyboard())
        else:
            await msg.reply("", reply_markup=main_keyboard())
    ...
    # ── CONFIRM CLEAN DATA ────────────────────────────────────────────────────
        ...
        if text.strip().upper() != "CLEAN":
            await msg.reply(
                "❌ Type exactly `CLEAN` to confirm. Or tap Cancel.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Cancel", callback_data="go_home")]])
            )
            return
        await db.clean_user_data(uid)
        await msg.reply(
            "🧹 **All your data has been cleaned!**\n\n"
            "✅ Token deleted\n✅ All repos deleted\n✅ All states cleared\n\n"
    ...
    # ── FILE MANAGER: DELETE paths ────────────────────────────────────────────
        ...
        results = []
        for p in paths:
            ok, res = git.delete_path_in_repo(repo_folder, p)
            results.append(res)
        await msg.reply(
            "🗑 **Delete Results:**\n\n" + "\n".join(results) +
            "\n\n💡 Use **🚀 Git Push** to sync to GitHub.",
            reply_markup=main_keyboard()
        )
    ...
    # ── FILE MANAGER: ADD FILE step 2 (content) ──────────────────────────────
        ...
        await db.clear_state(uid)
        ok, result = git.write_file_in_repo(repo_folder, file_path, msg.text or "")
    ...
    # ── FILE MANAGER: MULTI ADD ───────────────────────────────────────────────
        ...
        if raw.upper() == "DONE":
            ...
            for fpath, fcontent in files.items():
                ok, res = git.write_file_in_repo(repo_folder, fpath, fcontent)
                results.append(res)
            await msg.reply(
                f"📚 **Multi-File Add Complete!**\n\n" + "\n".join(results) +
                f"\n\nTotal: **{len(files)} file(s)** added/updated\n\n"
                "💡 Use **🚀 Git Push** to sync to GitHub.",
                reply_markup=main_keyboard()
            )
            return
        ...
        if current_path is None:
            ...
            await msg.reply(f"📄 Path: `{repo_folder}/{path}`\n\nNow send the **content** for this file:")
        else:
            files[current_path] = raw
            count = len(files)
            await db.set_state(uid, {**state, "files": files, "current_path": None})
            await msg.reply(
                f"✅ **File {count} saved:** `{current_path}`\n\n"
                f"Files queued: {count}\n"
                "Send **next file path** or `DONE` to write all files:"
            )
...
if __name__ == "__main__":
    print(f"\n{'='*52}")
    print(f"  🤖 GitHub Control Bot v4.0")
